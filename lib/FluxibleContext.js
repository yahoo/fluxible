 ...
 * @param {Object} meta Metadata to keep track of the current flow of executing actions
 * @param {Function} resolve function to call on success
 * @param {Function} reject function to call on failure
 * @private
 */
function executeActionInternal(context, action, payload, meta, resolve, reject) {
    var actionContext = context.getActionContext(meta);
    var displayName = action.displayName || action.name;
    debug('Executing action ' + displayName + ' with trace', actionContext.stack.join('.'));
 ...
 */
function executeActionProxy (self, action, payload, meta, done) {
    var displayName = action.displayName || action.name;
    payload = (undefined !== payload) ? payload : {};
    meta = meta || {
        stack: ['Action', displayName],
        id: generateUUID()
    }
    if (__DEV__) {
        if (!action) {
    ...
    if (__DEV__) {
        if (self._dispatcher && self._dispatcher.currentAction) {
            var currentActionDisplayName = self._dispatcher.currentAction.displayName ||
                self._dispatcher.currentAction.name;
    ...
    var executeActionPromise = new Promise(function executeActionPromise(resolve, reject) {
        ...
        setImmediate(function () {
            try {
                executeActionInternal(self, action, payload, meta, resolve, reject);
            } catch (err) {
                reject(err);
            }
        });
...
FluxContext.prototype.executeAction = function executeAction(action, payload, done) {
    return executeActionProxy(this, action, payload, null, done);
};
 ...
 */
FluxContext.prototype.getActionContext = function getActionContext(meta) {
    var self = this;
    ...
    if (!self._actionContext) {
        ...
        var actionContext = {
            ...
            executeAction: function actionExecuteAction (action, payload, callback) {
                /*
                 * When an action calls an action,
                 * add the new action's name to the stack.
                 * We have to clone the stack (concat), can't just push to it
                 * since one action can execute multiple actions.
                 * `this` is a reference to the object returned after the
                 * objectAssign below.
                 */
                var actionName = action.displayName || action.name;
                var newStack = this.stack.concat([actionName]);
                return executeActionProxy(self, action, payload, {
                    stack: newStack,
                    id: this.id
                }, callback);
            },
     ...
     * We expect mext to be defined in most cases (by executeActionInternal)
     * unless the user calls `getActionContext` manually.
     */
    return objectAssign({}, self._actionContext, {
        stack: (meta && meta.stack) || ['Action'],
        id: (meta && meta.id) || generateUUID()
    });
};
