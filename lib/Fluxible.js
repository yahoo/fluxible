...
Fluxible.prototype.rehydrate = function rehydrate(obj, callback) {
    ...
    var pluginTasks = this._plugins.filter(function (plugin) {
        ...
        if (2 === plugin.rehydrate.length) { // Async plugin
            return PromiseLib.promisify(plugin.rehydrate.bind(plugin))(obj.plugins[plugin.name]);
        }
    ...
    PromiseLib.all(pluginTasks).then(function rehydratePluginTasks() {
        var contextRehydrate = PromiseLib.promisify(context.rehydrate.bind(context));
        return contextRehydrate(obj.context);
    }).done(function () {
